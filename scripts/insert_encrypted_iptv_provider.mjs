...
db.prepare(`INSERT INTO iptv_providers (id, name, type, server_url, encrypted_credentials, expires_at, enabled, health_status, last_refresh_at, total_channels, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
  .run(id, 'insert-test', 'xtream', 'http://example.com', enc, null, 1, 'unknown', null, 0, now, now);

const row = db.prepare('SELECT id, encrypted_credentials FROM iptv_providers WHERE id = ?').get(id);
console.log('inserted id=', row.id);
console.log('encrypted_credentials sample=', row.encrypted_credentials?.slice(0,60));
