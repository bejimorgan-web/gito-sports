...
class MobileApiService {
  ...
  Future<List<MobileCompetition>> getCompetitions(
    ...
    final path =
        '/mobile/competitions${query.isEmpty ? '' : '?${Uri(queryParameters: query).query}'}';
  ...
  Future<List<MobileCompetition>> getCompetitionsForHost(
          String sportId, String hostId) async =>
  ...
  Future<List<MobilePublishedPublication>> getPublishedPublications() async =>
      ((await _get('/mobile/publications')) as List)
          .whereType<Map>()
          .map((item) => MobilePublishedPublication.fromJson(
  ...
  Future<MobileFixture> getFixture(String fixtureId, {String? clubId}) async =>
      MobileFixture.fromJson(Map<String, dynamic>.from(await _get(
          '/mobile/fixtures/$fixtureId${clubId == null ? '' : '?clubId=${Uri.encodeQueryComponent(clubId)}'}')));
