...
export function createPlayer(input: CreatePlayerRequest): Player {
  ...
  database.prepare("INSERT INTO players (id, team_id, country_id, first_name, last_name, display_name, position, jersey_number, height_cm, weight_kg, birth_date, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)").run(playerId, input.teamId, input.countryId ?? null, firstName, lastName, input.displayName?.trim() || `${firstName} ${lastName}`, input.position ?? null, input.jerseyNumber ?? null, input.heightCm ?? null, input.weightKg ?? null, input.birthDate ?? null, timestamp, timestamp);
