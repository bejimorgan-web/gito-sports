...
export function NewsClassificationPanel({ classification, labelFor, selectedIds, onToggle, onApprove, onReject, onApproveSelected, onRerun, aiStatus = "idle", onAiClassify, entities, onRemoveApproved, onAddManual, approvingCategoryId, approvingMultiple }: Props) {
  ...
  return <section className="console-panel news-classification-panel">
    ...
    <div className="news-classification-approved"><h4>Approved</h4>
      {(["team", "competition", "country", "host", "sport", "match"] as const).map((type) => <CategoryGroup key={type} title={({ team: "Teams", competition: "Competitions", country: "Countries", host: "Hosts", sport: "Sports", match: "Matches" })[type]} categories={group(approved, type)} labelFor={labelFor} onRemove={onRemoveApproved} />)}
    </div>
