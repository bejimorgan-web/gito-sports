...
class StateResolver {
    static instance;
    stateTracking = new Map();
    eventBlockWindowMs = 5000; // Block API/sync updates for 5s after event
    conflictCount = 0;
    resolveCount = 0;
    constructor() { }
    static getInstance() {
     ...
     */
    resolve(key, value, source, eventId) {
        this.resolveCount += 1;
        ...
        if (source === "background-sync") {
            if (tracking.current?.source === "event" ||
    ...
    getStats() {
        return {
            ...
            conflictRate: this.resolveCount > 0 ? ((this.conflictCount / this.resolveCount) * 100).toFixed(2) + "%" : "0%",
...
export function getStateResolver() {
