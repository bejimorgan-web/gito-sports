...
class GlobalInvalidation {
    static instance;
    apiBaseUrl;
    iptvCallbacks = new Set();
    scoresCallbacks = new Set();
    streamsCallbacks = new Set();
     ...
     */
    async invalidateIPTV() {
        console.log("[GlobalInvalidation] IPTV invalidated, triggering refetch");
        const promises = Array.from(this.iptvCallbacks).map((cb) => Promise.resolve(cb()).catch((err) => console.error("[GlobalInvalidation] IPTV refetch error", err)));
        await Promise.all(promises);
    }
    /**
     * Trigger scores refetch via all registered callbacks
     */
    async invalidateScores() {
        console.log("[GlobalInvalidation] Scores invalidated, triggering refetch");
        const promises = Array.from(this.scoresCallbacks).map((cb) => Promise.resolve(cb()).catch((err) => console.error("[GlobalInvalidation] Scores refetch error", err)));
        await Promise.all(promises);
    }
    /**
     * Trigger streams refetch via all registered callbacks
     */
    async invalidateStreams() {
        console.log("[GlobalInvalidation] Streams invalidated, triggering refetch");
        const promises = Array.from(this.streamsCallbacks).map((cb) => Promise.resolve(cb()).catch((err) => console.error("[GlobalInvalidation] Streams refetch error", err)));
        await Promise.all(promises);
    }
}
export function getGlobalInvalidation(apiBaseUrl) {
