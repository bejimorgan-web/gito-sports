...
test("AI classification action is explicit and suggestions remain visibly non-authoritative", async () => {
    ...
    const view = render(_jsx(NewsClassificationPanel, { classification: { approved: [], suggestions: [suggestion] }, labelFor: () => "Bayern Munich", selectedIds: [], onToggle: () => undefined, onApprove: () => undefined, onReject: () => undefined, onApproveSelected: () => undefined, onRerun: () => undefined, onAiClassify: () => { aiCalls += 1; } }));
