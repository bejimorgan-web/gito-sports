export function getSelectedCategoryEntityId(rows, categoryType) {
    return rows.find((row) => row.categoryType === categoryType)?.entityId || undefined;
}
export function getFilteredCategoryOptions(input) {
    ...
    const competitionIdsForSport = new Set(input.competitions
        .filter((competition) => !selectedSportId || competition.sportId === selectedSportId)
        .map((competition) => competition.id));
    const competitionTeamIds = new Set(selectedCompetitionId && input.competitionTeamIds ? (input.competitionTeamIds[selectedCompetitionId] ?? []) : []);
    switch (input.categoryType) {
        ...
        case "country": {
            ...
            const sportCountryIds = new Set((input.sports.find((sport) => sport.id === selectedSportId)?.countryIds ?? []));
...
export function sanitizeNewsCategoryRows(rows, input) {
    ...
    const competitionIdsForSport = new Set(input.competitions
        .filter((competition) => !selectedSportId || competition.sportId === selectedSportId)
        .map((competition) => competition.id));
    const currentCompetitionTeamIds = new Set(selectedCompetitionId && input.competitionTeamIds ? (input.competitionTeamIds[selectedCompetitionId] ?? []) : []);
    return rows.map((row) => {
        if (!row.entityId) {
            return row;
        }
