...
import { filterNewsArticles, getNormalizedFetchedSourcePreview, isGiTOFactCheckRequired, isGiTOGeneratedArticle, isGiTOInsufficientSourceMaterial, shouldClearEditingArticleAfterDelete, shouldClearSelectedArticleAfterDelete } from "./news-workspace-helpers";
...
function buildFetchMock(initialArticles, generatedCrawlerTier = "http") {
    const articles = initialArticles.map((article) => ({ ...article }));
    const deleteRequests = [];
    const researchRequests = [];
    let resolveResearchRequest = null;
    const generationRequests = [];
    const publishRequests = [];
    let resolveGenerationRequest = null;
    const rssSources = [];
    const rssRequests = [];
    const generatedFeeds = [];
    let resolveGeneratedFeed = null;
    let generatedFeedError = null;
    const sources = [
    ...
    const createJsonResponse = (body, status = 200) => new Response(JSON.stringify(body), {
        status,
    ...
    const fetchMock = async (input, init) => {
        ...
        if (pathname === "/news/generated-rss-sources" && method === "GET")
            return createJsonResponse(generatedFeeds);
        if (pathname === "/news/generated-rss-sources" && method === "POST") {
            if (generatedFeedError)
                return createJsonResponse({ error: "rss_fetch_failed_403", message: generatedFeedError }, 400);
            const payload = JSON.parse(typeof init?.body === "string" ? init.body : "{}");
        ...
        if (pathname === "/news/generated-rss-sources/web-feed-1/refresh" && method === "POST")
            return createJsonResponse({ ...generatedFeeds[0], status: "ready", discoveredArticleCount: 3 });
        if (pathname === "/news/generated-rss-sources/web-feed-1/articles" && method === "GET")
            return createJsonResponse([{ id: "web-article-1", title: "Discovered story", sourceName: "Example Sports", articleUrl: "https://example.com/story", publishedAt: "2026-08-19T10:00:00.000Z" }]);
        if (pathname === "/news/generated-rss-sources/web-feed-1" && method === "DELETE") {
            generatedFeeds.splice(0, generatedFeeds.length);
            return new Response(null, { status: 204 });
        }
        ...
        if (pathname === "/news/articles/bulk-delete" && method === "POST") {
            const bodyText = typeof init?.body === "string" ? init.body : "";
            const payload = bodyText ? JSON.parse(bodyText) : { ids: [] };
    ...
    return {
        ...
        setGeneratedFeedError: (message) => { generatedFeedError = message; }
    };
}
test("isGiTOGeneratedArticle returns false for null or non-AI articles", () => {
    assert.equal(isGiTOGeneratedArticle(null), false);
    assert.equal(isGiTOGeneratedArticle({ id: "1", title: "Test", status: "draft" }), false);
});
test("isGiTOGeneratedArticle returns true for explicit GiTO AI origin or author", () => {
    assert.equal(isGiTOGeneratedArticle({ id: "1", title: "AI Draft", status: "review", contentOrigin: "gito_ai" }), true);
    assert.equal(isGiTOGeneratedArticle({ id: "2", title: "AI Draft", status: "review", author: "GiTO News" }), true);
});
test("isGiTOFactCheckRequired recognizes gito-fact-check-needed tags", () => {
    assert.equal(isGiTOFactCheckRequired(null), false);
    assert.equal(isGiTOFactCheckRequired({ id: "1", title: "Test", status: "draft", tags: ["other-tag"] }), false);
    assert.equal(isGiTOFactCheckRequired({ id: "2", title: "Test", status: "review", tags: ["gito-fact-check-needed"] }), true);
});
...
test("isGiTOInsufficientSourceMaterial recognizes insufficient-source metadata", () => {
    ...
    assert.equal(isGiTOInsufficientSourceMaterial({ id: "1", title: "Test", status: "review", tags: ["gito-fact-check-needed"] }), false);
    assert.equal(isGiTOInsufficientSourceMaterial({ id: "2", title: "Test", status: "review", tags: ["gito-insufficient-source-material"] }), true);
});
test("filterNewsArticles filters by search and special GiTO statuses", () => {
    const articles = [
        { id: "1", title: "Local match report", status: "review", source: { id: "source-a" }, tags: ["gito-fact-check-needed"] },
        { id: "2", title: "GiTO AI Draft", status: "draft", source: { id: "source-b" }, contentOrigin: "gito_ai", tags: [] },
        { id: "3", title: "Insufficient feed", status: "draft", source: { id: "source-a" }, tags: ["gito-insufficient-source-material"] }
    ];
...
test("incoming News filters and Select visible bulk selection work together", async () => {
    ...
    try {
        render(_jsx(NewsWorkspaceScreen, { accessToken: "dummy-token" }));
        ...
        const incoming = within(incomingSection);
        ...
        const searchInput = incoming.getByDisplayValue("Search");
...
test("rendered News research workflow produces a reviewed GiTO story and preserves incoming bulk controls", async () => {
    ...
    globalThis.fetch = fetchMock;
    try {
        const view = render(_jsx(NewsWorkspaceScreen, { accessToken: "dummy-token" }));
        ...
        const incoming = within(incomingSection.closest("section"));
        ...
        const summaryInput = view.getByLabelText("Summary");
        const bodyInput = view.getByLabelText("Body");
        ...
        fireEvent.click(view.getByRole("button", { name: "Incoming News" }));
        const incomingAgain = within((await view.findByRole("heading", { name: "Incoming News" })).closest("section"));
...
test("rendered News RSS sources supports add, fetch, publisher URL, and delete", async () => {
    ...
    try {
        ...
        fireEvent.submit(view.getByRole("button", { name: "Add source" }).closest("form"));
...
test("rendered Web Page RSS workflow supports deferred generate, preview, refresh, copy, and delete", async () => {
    ...
    const originalConfirm = window.confirm;
    let copiedText = "";
    Object.defineProperty(window.navigator, "clipboard", { value: { writeText: async (value) => { copiedText = value; } }, configurable: true });
    globalThis.fetch = fetchMock;
    window.confirm = () => true;
    try {
        const view = render(_jsx(NewsWorkspaceScreen, { accessToken: "dummy-token" }));
        ...
        fireEvent.submit(view.getByRole("button", { name: "Generate RSS Feed" }).closest("form"));
