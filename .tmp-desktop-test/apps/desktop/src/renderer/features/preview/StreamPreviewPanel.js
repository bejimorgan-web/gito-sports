...
function redactPreviewUrl(value) {
    if (!value)
        return value ?? null;
    try {
...
export const StreamPreviewPanel = memo(function StreamPreviewPanel({ channel, playbackEntity, providerType, compact = false, onHealthChange, onPreviewReady, apiBaseUrl }) {
    ...
    const lastChannelRef = useRef();
    const previousPropChannelRef = useRef();
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const previewTrace = (type, detail) => {
            if (!diagnosticsEnabled)
                return;
            const message = `GITO_PREVIEW_TRACE: ${type}${detail ? ` - ${detail}` : ""}`;
            // eslint-disable-next-line no-console
            console.info(message);
            const logs = window.__GITO_PREVIEW_LOGS__;
            if (Array.isArray(logs)) {
                logs.push(message);
            }
            else {
                window.__GITO_PREVIEW_LOGS__ = [message];
            }
        };
        if (!video || !channel) {
        ...
        const nativeDiagnosticEvents = ["loadstart", "loadedmetadata", "loadeddata", "canplay", "canplaythrough", "play", "pause", "suspend", "seeking", "seeked", "emptied", "durationchange", "progress", "abort"];
        const nativeDiagnosticHandlers = new Map();
        ...
        function mountStream(url, channelId) {
            window.clearTimeout(stallTimer);
            ...
            if (Hls.isSupported() && url.includes(".m3u8")) {
                ...
                hlsRef.current = hls;
                const logHlsEvent = (eventName, data) => {
                ...
                for (const [eventName, eventKey] of [
                    ...
                    hls.on(eventKey, (_event, data) => logHlsEvent(eventName, data));
                }
                hls.loadSource(url);
        ...
        if (apiBaseUrl) {
            try {
                ...
                const unsubStreamRecovered = eventClient.on("stream:recovered", (payload) => {
                ...
                const unsubStreamReconnected = eventClient.on("stream:reconnected", (payload) => {
    ...
    useEffect(() => {
        const video = videoRef.current;
        if (!playbackEntity || !video || !window.gito?.desktopPlayback)
            return undefined;
        let disposed = false;
        let cleanupMedia;
        let hls;
        ...
        const start = async () => {
            try {
                ...
                try {
                    ...
                    hls = new Hls({ loader: Loader, pLoader: Loader, fLoader: Loader });
                ...
                video.__gitoPlaybackSessionId = session.sessionId;
            }
            catch (error) {
                if (!disposed) {
        ...
        return () => {
            ...
            hls?.destroy();
            cleanupMedia?.();
            const sessionId = video.__gitoPlaybackSessionId;
            if (sessionId)
                void playback.cancel(sessionId);
            delete video.__gitoPlaybackSessionId;
        };
    }, [onHealthChange, onPreviewReady, playbackEntity, updateStatus]);
    return (_jsxs("section", { className: compact ? "preview-panel compact-preview" : "screen-stack", children: [!compact ? (_jsxs("header", { className: "screen-header", children: [_jsx("p", { className: "eyebrow", children: "Preview" }), _jsx("h2", { children: "Live Stream Preview" }), _jsx("span", { children: "Switch channels and validate playback before approval." })] })) : null, _jsxs("div", { className: "preview-layout", children: [_jsx("section", { className: "video-shell", children: channel || playbackEntity ? (_jsx("video", { ref: videoRef, controls: true, playsInline: true, preload: "metadata" })) : (_jsx("div", { className: "video-placeholder", children: "Select a channel to preview" })) }), _jsxs("aside", { className: "console-panel", children: [_jsxs("div", { className: "panel-heading", children: [_jsx("h3", { children: "Stream Status" }), _jsx("span", { className: "status-pill", children: status })] }), retryAvailable ? (_jsx("div", { className: "retry-action", children: _jsx("button", { type: "button", className: "retry-button", onClick: () => {
                                        setRetryAvailable(false);
                                        setRetryRequestKey((current) => current + 1);
                                        setPlaybackState("recovering");
                                        updateStatus("Retrying stream...");
                                    }, children: "Reconnect stream" }) })) : null, _jsxs("dl", { children: [_jsx("dt", { children: "Channel" }), _jsx("dd", { children: channel?.name ?? (playbackEntity ? `${playbackEntity.entityType} ${playbackEntity.entityId}` : "Not selected") }), _jsx("dt", { children: "Category" }), _jsx("dd", { children: channel?.groupName ?? (playbackEntity ? "Desktop playback" : "Not selected") }), _jsx("dt", { children: "Playback URL" }), _jsx("dd", { className: "break-word", children: channel ? channel.url : playbackEntity ? "Secure playback session" : "Not selected" })] })] })] })] }));
});
