...
function jsonResponse(payload, contentType = "application/json") { return new Response(JSON.stringify(payload), { status: 200, headers: { "content-type": contentType } }); }
    ...
    try {
        ...
        globalThis.fetch = (async (input) => {
            ...
            if (url.searchParams.get("action") === "get_vod_streams")
                return jsonResponse([{ stream_id: "movie-1", stream_url: "https://synthetic.invalid/authenticated/movie.m3u8" }]);
            if (url.toString() === "https://synthetic.invalid/authenticated/movie.m3u8")
            ...
            if (url.toString() === "https://synthetic.invalid/secret/key")
                return new Response("synthetic-key", { status: 200, headers: { "content-type": "application/octet-stream" } });
            if (url.toString() === "https://synthetic.invalid/init.mp4")
                return new Response("synthetic-init", { status: 200, headers: { "content-type": "video/mp4" } });
            if (url.toString() === "https://synthetic.invalid/authenticated/segment.ts")
                return new Response("synthetic-segment", { status: 200, headers: { "content-type": "video/mp2t" } });
...
test("production transport rejects unsafe inputs, isolates resources, and invalidates cancellation/expiry", async () => {
    ...
    try {
        ...
        const second = await expiring.start({ entityType: "movie", entityId: movie.id });
        now += 601_000;
...
test("production non-HLS transport returns bounded 206 ranges without exposing the source URL", async () => {
    ...
    try {
        ...
        globalThis.fetch = (async (input, init) => {
            const url = new URL(String(input));
            if (url.hostname === "playlist.invalid")
