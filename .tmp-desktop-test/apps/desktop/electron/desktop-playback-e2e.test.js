...
function cleanup(store, file) { store.close(); fs.rmSync(file, { force: true }); for (const suffix of ["-wal", "-shm"])
    fs.rmSync(`${file}${suffix}`, { force: true }); }
function response(body, contentType) { return new Response(body, { status: 200, headers: { "content-type": contentType } }); }
function manifestResources(text) { return [...text.matchAll(/gito-resource:\/\/(resource-[A-Za-z0-9_-]+)/g)].map((match) => match[1]); }
    ...
    globalThis.fetch = (async (input, init) => {
        ...
        if (url.hostname === "synthetic-provider.invalid") {
            ...
            if (action === "get_vod_streams")
                return response(JSON.stringify([{ stream_id: "movie-1", stream_url: "https://synthetic-provider.invalid/movie/master.m3u8" }]), "application/json");
            if (action === "get_series_info")
                return response(JSON.stringify({ episodes: { "1": [{ id: "episode-1", movie_url: "https://synthetic-provider.invalid/episode/master.m3u8" }] } }), "application/json");
            if (url.pathname === "/movie/master.m3u8" || url.pathname === "/episode/master.m3u8")
                return response("#EXTM3U\n#EXT-X-STREAM-INF:BANDWIDTH=1000\nvariant.m3u8\n", "application/vnd.apple.mpegurl");
            if (url.pathname === "/movie/variant.m3u8" || url.pathname === "/episode/variant.m3u8")
                return response("#EXTM3U\n#EXT-X-KEY:METHOD=AES-128,URI=\"key.bin\"\n#EXT-X-MAP:URI=\"init.mp4\"\nsegment-001.m4s\n", "application/vnd.apple.mpegurl");
            if (url.pathname.endsWith("/key.bin"))
                return response("synthetic-key", "application/octet-stream");
            if (url.pathname.endsWith("/init.mp4"))
                return response("synthetic-init", "video/mp4");
            if (url.pathname.endsWith("/segment-001.m4s"))
                return response("synthetic-segment", "video/iso.segment");
        }
        if (url.hostname === "synthetic-m3u.invalid") {
            if (url.pathname === "/movie/master.m3u8")
                return response("#EXTM3U\n#EXT-X-STREAM-INF:BANDWIDTH=1000\nvariant.m3u8\n", "application/vnd.apple.mpegurl");
            if (url.pathname === "/movie/variant.m3u8")
                return response("#EXTM3U\n#EXT-X-KEY:METHOD=AES-128,URI=\"key.bin\"\n#EXT-X-MAP:URI=\"init.mp4\"\nsegment-001.m4s\n", "application/vnd.apple.mpegurl");
            if (url.pathname.endsWith("/key.bin"))
                return response("synthetic-key", "application/octet-stream");
            if (url.pathname.endsWith("/init.mp4"))
...
async function traverse(handlers, sessionId) {
...
test("production handler E2E traverses synthetic Movie and Episode HLS resources", async () => {
    const file = tempPath();
    const store = new DesktopSqliteStore(file);
    const credentials = new MemoryCredentialStore();
    const restore = installProviderFixture();
    try {
        ...
        for (const entity of [{ entityType: "movie", entityId: movie.id }, { entityType: "episode", entityId: episode.id }]) {
            ...
            loader.load({ url: "gito-resource://resource-001", type: "manifest" }, {}, {
                onSuccess: (data) => { loaderData = data.data; },
                onError: (error) => { throw error; }
            });
            for (let attempt = 0; attempt < 20 && !loaderData; attempt += 1)
...
test("malicious renderer requests and cross-session resources are rejected", async () => {
    ...
    try {
        ...
        const a = await handlers.playbackStart({ entityType: "movie", entityId: movie.id });
        const b = await handlers.playbackStart({ entityType: "movie", entityId: movie.id });
